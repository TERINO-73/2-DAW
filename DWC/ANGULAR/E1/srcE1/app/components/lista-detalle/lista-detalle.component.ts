...
export class ListaDetalleComponent {
    ...
    public facturaId = parseInt(this.aR.snapshot.params["id"]);
    public facturaNumero = this.aR.snapshot.params["numero"];
    ...
    onSubmit() {
        if (this.detalleId == -1) {
            ...
            this.pAJAX.editarDetalle(this.detalle, this.detalle.id_factura, this.detalle.cantidad, this.detalle.concepto, this.detalle.precio, this.detalle.tipo_iva).subscribe(respuesta => {
    ...
    borrar(detalleId: number) {
        if (confirm("¿Desea borrar el detalle con id " + detalleId + "?")) {
