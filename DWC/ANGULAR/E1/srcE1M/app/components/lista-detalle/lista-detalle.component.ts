...
export class ListaDetalleComponent {
    ...
    public facturaId = this.aR.snapshot.params["id"];
    ...
    onSubmit() {
        if (this.detalleId == -1) {
            this.pAJAX.insertarDetalle(this.detalle).subscribe(datos => {
                this.detalles = datos;
                this.mostrarFormulario = false;
            })
        } else {
            // this.detalle.id = this.detalleId;
            this.pAJAX.editarDetalle(this.detalle).subscribe(respuesta => {
